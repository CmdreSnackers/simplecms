...
if ( $_SERVER["REQUEST_METHOD"] === 'POST' ) {
    ...
    // step 3: do error check
      ...
      // do check to update password or not 
    ...
    // if passowrd is updated
    if($is_password_changed) {
      $rules['password'] = 'password_check'; //make sure length is >=8
    ...
    //if email is changed, prevent dupe
    //check for email changes
    if($user['email'] !== $_POST['email']) {
      // do db check if email is already there
    ...
    CSRF::removeToken('edit_user_form');
        ...
        // step 6 redirect to manage-user
